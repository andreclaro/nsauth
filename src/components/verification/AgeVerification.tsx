...
export function AgeVerification() {
  ...
  const handleStartVerification = async () => {
    ...
    try {
      const { url, onResult } = await zkPassportService.requestAgeVerification();
      ...
      onResult(async (result: VerificationCallbackResult) => {
        ...
        if (result.verified) {
          const proof = zkPassportService.processVerificationResult('age', result);
          if (proof) {
      ...
      console.error('Failed to start age verification:', err);
  ...
  if (verified) {
    return (
      <div className="verification-status verified">
        ...
        <span className="status-text">Age verified (18+)</span>
      </div>
    );
  }
      ...
      {!verificationUrl ? (
        ...
        <div className="verification-qr-container">
          <p className="qr-instructions">
            Scan this QR code with the ZKPassport app to verify your age:
          </p>
